		...
		while rclpy.ok():
			for ep in range(1, TOTAL_EPISODES):
				...
				for t in range(MAX_STEP):
					...
					print(f"action is: linear: {action[1]:.2f}, agnular: {action[0]:.2f}")
					...
					if t%5 == 0:
						...
						c, a = agent.learn(agent.buffer.get_batch(unbalance_p=UNBALANCE_P))
					...
					if done == 1:
						...
						h, m = divmod(m, 60)
						print(f"Episode: {ep} score: {total_reward:.2f} memory: {len(agent.buffer.buffer)} time: {h}:{m:02d}:{s:02d}")
						break
