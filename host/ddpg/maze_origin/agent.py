...
class AgentDDPG:
	...
	def save_model(self, num_trials, trial_len):
		...
		self.actor_network.save('actor_model' + '_' + str(num_trials) + '.keras', overwrite=True)
		self.critic_network.save('critic_model' + '_' + str(num_trials) + '.keras', overwrite=True)
	...
	def load_weights(self, path):
		...
		"""
		try:
			self.actor_network.load_weights(path + ".weights.h5")
			self.critic_network.load_weights(path + ".weights.h5")
			self.critic_target.load_weights(path + ".weights.h5")
			self.actor_target.load_weights(path + ".weights.h5")
