...
class GazeboEnv(Node):
    def __init__(self, action_size):
        ...
        self.delete_entiry_proxy = self.create_client(DeleteEntity, '/delete_entity')

    def delete_goal(self):
        while not self.delete_entiry_proxy.wait_for_service(timeout_sec=1.0):
            print('delete_goal: service /delete_entity not available, waiting again...')
        request = DeleteEntity.Request()
        request.name = self.entity_name
        future = self.delete_entiry_proxy.call_async(request)
    ...
    def set_new_goal(self, init_goal=False):
        if init_goal:
            ...
            # world2 + empty world
            # goal_x_list = [-1.5, -1.5, -1.5, -1.5, -0.5, -0.5, -0.5, -0.5, 0.5, 0.5, 0.5, 0.5, 1.5, 1.5, 1.5, 1.5,]
            # goal_y_list = [1.5, 0.5, -0.5, -1.5, 1.5, 0.5, -0.5, -1.5, 1.5, 0.5, -0.5, -1.5, 1.5, 0.5, -0.5, -1.5,]

            # MASE
            goal_x_list = [-4.0, -4.0, -4.0, 4.0, 4.0, 4.0, 4.0, -1.0, 4.0]
            goal_y_list = [ 0.0,  2.0, -2.0, 3.0, 4.0, -3.0, -4.0, 4.0, 0.0]

            position_ok = False
            while not position_ok:
                self.index = random.randrange(0, 16)
...
if __name__ == '__main__':
    ...
    agent = keras.saving.load_model("stage_1_50.keras", custom_objects=None, compile=True, safe_mode=True)
    ...
    try:
        while rclpy.ok():
            for ep in range(1, 10):
                env.get_logger().info(f"================================== Episode: {ep} ==================================")
                done = False
                state = env.reset()
                for t in range(500):
                    q_value =agent.predict(state.reshape(1, len(state)), verbose=0)
                    action = np.argmax(q_value[0])
                    next_state, reward, done = env.step(action)
                    env.get_logger().info(f"step {t}, reward: {reward}")
